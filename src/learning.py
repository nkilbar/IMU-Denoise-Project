...
class LearningBasedProcessing:
    ...
    def train(self, dataset_class, dataset_params, train_params):
        ...
        # Training losses per Epoch defined here
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            if epoch % freq_val == 0:
                ...
                best_loss = write_val(loss, best_loss) #this outputs single validation loss here
                start_time = time.time()
                vlosslist.append(loss); 
            # Collect in a list
            epochslist.append(epoch); #
            tlosslist.append(loss_epoch); #
        ...
        file_path = 'vloss.txt'
        file_path2 = 'epochs.txt'
        file_path3 = 'tloss.txt'
        ...
        with open(file_path2, 'w') as file:
            # Iterate through the list and write each element to a new line
            for item in epochslist:
                file.write(str(item) + '\n')
        with open(file_path3, 'w') as file:
            # Iterate through the list and write each element to a new line
            for item in tlosslist:
                file.write(str(item) + '\n')
    ...
    def test(self, dataset_class, dataset_params, modes):
        ...
        # plt.plot(epochslist, loss_epoch, color='black', label=r'Training Loss')
        # plt.plot(epochslist, test_loss, color='blue', label=r'Validation Loss')
...
class GyroLearningBasedProcessing(LearningBasedProcessing):
    ...
    def plot_orientation_error(self, imu_Rots, net_Rots, N):
        ...
        for i in range(3):
            ...
            # print(torch.sqrt(torch.mean(net_err[:, i]**2)))
    ...
    def plot_gyro_correction(self):
        ...
        # print(self.ts)
        # print(self.gyro_corrections)
        # plt.plot(self.ts, self.gyro_corrections, label=r'net IMU')
        plt.plot(self.ts.cpu().numpy(),self.gyro_corrections.cpu().numpy(), label=r'net IMU')
...
# # Plot training loss
# plt.subplot(1, 2, 1)
# plt.plot(epochslist, losslist, label='Training Loss')
...
# plt.subplot(1, 2, 2)
# plt.plot(epochslist, test_loss, label='Validation Loss', color='orange')
